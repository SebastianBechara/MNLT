...
sub _check_mandatory_parameters {
  ...
  if($self->auto) {
     ...
     if($self->{USE_INSERT_REFERENCE}) {
        ...
        $self->{IES} = $self->{OUT_DIR}."/Insert/Insert.gff3" if(-e $self->{OUT_DIR}."/Insert/Insert.gff3");
...
#			=> Map the sequences on the new ref (BOWTIE2)
	my ($new_mapping, $new_ref)=$self->_bowtie2_mapping($seq_id, \$current_seq, $file2locate);#\$current_seq, $file2locate);
...
#			=> Use the get_IES_from_aln
	...
	foreach my $i (sort {$segments{$seq_id}->{$a}->{start}<=>$segments{$seq_id}->{$b}->{start}} keys %{$segments{$seq_id}}){
		...
		if(defined($results{$indel_id})){
			...
			$results{$indel_id}->{read_names}=join(',',$results{$indel_id}->{read_names}, @{$segments{$seq_id}->{$i}->{read_names}}) if($isayso==1);
		}
		else{
			$results{$indel_id}={
				...
				type => $segments{$seq_id}->{$i}->{type},
				subtype => $segments{$seq_id}->{$i}->{subtype},
				ies_id => $segments{$seq_id}->{$i}->{ies_id},
				ies_min_dist => $segments{$seq_id}->{$i}->{ies_min_dist},
				ies_left_dist => $segments{$seq_id}->{$i}->{ies_left_dist},
				ies_right_dist => $segments{$seq_id}->{$i}->{ies_right_dist},
				common_boundary => $segments{$seq_id}->{$i}->{common_boundary},
				support_ref => $segments{$seq_id}->{$i}->{support_ref},
				support_variant => $segments{$seq_id}->{$i}->{support_variant},
			};
			$results{$indel_id}->{read_names} = join(',', @{$segments{$seq_id}->{$i}->{read_names}}) if($isayso==1);
		}
	}
...
sub _bowtie2_mapping{
	...
	if(-s $file2locate){
		...
		system("$bowtie2 --threads 1 --end-to-end --quiet -f  --very-sensitive -k 2 -x $file_ref -U $file2locate | $samtools view -F 4 -uS - 2> /dev/null | $samtools sort -o $file_bam.bam -  > /dev/null 2>&1");
...
sub _refine_segments{
	...
	foreach my $seg_id (keys %{$segments}){
	   ...
	   my $qname = $seq_id;
		...
		my ($mv_lft, $i)=(0,1);
		while(substr($taln_left_flank, length($taln_left_flank)-$i,1) eq substr($segments->{$seg_id}->{sequence}, length($segments->{$seg_id}->{sequence})-$i,1)){
				$mv_lft++;
				$i++;
		}
...
sub _compare_segment_to_ies{
	my ($self, $seq_id, $cur_seg)=@_;
	my $start=$cur_seg->{start};
	my $end=$cur_seg->{end};
	my %ies;
	my $min_dist=1e30;

	$cur_seg->{type}="CRYPTIC";#=["crytptic", "alternative_boundary", "IES"];
	$cur_seg->{subtype}="MAC";#=["coding", "non_coding", "internal", "external", "overlapping", "IES"]
	$cur_seg->{ies_min_dist}="NA";
	$cur_seg->{ies_left_dist}="NA";
	$cur_seg->{ies_right_dist}="NA";
	$cur_seg->{common_boundary}="None";
	$cur_seg->{ies_id}="NA";

	my $closest_IES_pos="NA";
	if(defined(${$self->{LOADED_IES}->{$seq_id}}[0]->{start})){
		my $ies;
		foreach $ies (sort {$a->{start}<=>$b->{start}} @{$self->{LOADED_IES}->{$seq_id}}) {
			my $position=$ies->{start};
			my ($istart, $iend) = ($ies->{start}, $ies->{end});
			$ies->{istart}=$istart;
			$ies->{iend} = $iend;
			my @dist=(abs($istart-$start), abs($iend-$end));
			my $shortdist=min(@dist);
			next if ($shortdist >= $min_dist);

			$min_dist=$shortdist;
			$closest_IES_pos=$ies;

		}
		my $cur_ies=$closest_IES_pos;
		my $istart=$cur_ies->{istart};
		my $iend  =$cur_ies->{iend};
		$cur_seg->{ies_min_dist}=get_min_dist($start, $end, $istart, $iend);
		$cur_seg->{ies_left_dist}=$istart-$start;
		$cur_seg->{ies_right_dist}=$iend-$end;
		$cur_seg->{ies_id}=$cur_ies->{attributes}->{ID}[0];


		if($cur_seg->{ies_left_dist} == 0 && $cur_seg->{ies_right_dist} == 0 ){ # THIS IS AN IES
			$cur_seg->{type}="IES";
			$cur_seg->{subtype}="IES";
			$cur_seg->{common_boundary}="Both";
		}
		elsif($istart > $end || $iend < $start){ # THIS IS A CRYPTIC TA INDEL
			$cur_seg->{type}="CRYPTIC";
			$cur_seg->{subtype}="MAC";
		}
		elsif($istart > $start && $iend < $end ){ # THIS IS AN EXTERNAL ALTERNATIVE BOUNDARY
			$cur_seg->{type}="ALTERNATIVE";
			$cur_seg->{subtype}="EXTERNAL";
		}
		elsif($istart < $start && $iend > $end ){ # THIS IS AN INTERNAL ALTERNATIVE BOUNDARY
			$cur_seg->{type}="ALTERNATIVE";
			$cur_seg->{subtype}="INTERNAL";
		}
		else{ # THIS IS AN OVERLAPPING ALTERNATIVE BOUNDARY
			$cur_seg->{type}="ALTERNATIVE";
			if($istart == $start){
				$cur_seg->{common_boundary}="LEFT";
				if( $end < $iend){$cur_seg->{subtype}="PARTIAL_INTERNAL";}
				if( $end > $iend){$cur_seg->{subtype}="PARTIAL_EXTERNAL";}
			}
			elsif($iend==($end)){
				$cur_seg->{common_boundary}="RIGHT";
				if($start< $istart){$cur_seg->{subtype}="PARTIAL_EXTERNAL";}
				if($start> $istart){$cur_seg->{subtype}="PARTIAL_INTERNAL";}
			}
			else{$cur_seg->{subtype}="OVERLAPPING";}
		}
	} else{return 1;}
}
